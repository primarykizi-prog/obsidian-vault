...
- 強い例：意味のない日なんて、マジでない。
  ...
  ★ ブログ導線ブリッジ文（1行）← ここに入れる（末尾ではない）
  ## [考察見出し]  ← 現実と深くつながる洞察・コアテーマ軸を投入（300〜400文字）
  ## [出口見出し]  ← 背中を押す・押しつけない（200〜300文字）

### 中盤ブリッジ文（体験談→考察の間に1行だけ）

体験談の見出しの末尾に、ブログ記事へのブリッジ文を**1行だけ**入れる。
読者が「で、どうすればいいの？」と思うタイミングで、ブログに具体的な解説があることをさりげなく示す。

**ブリッジ文の例：**
- 「この"見えない仕事"の具体的な中身や、チーム医療の裏側については、ブログの方にもう少し詳しく書いています」
- 「こうした心のガードについて、もう少し掘り下げた記事をブログに置きました」

**ルール：**
- 1記事につき1箇所だけ（中盤のこの位置のみ）
- ブリッジ文の直後にブログ記事タイトル＋URLリンクを添える
- 押しつけない・「こちらもどうぞ！」のようなテンションにしない
- **末尾にはブログリンクを置かない**（締めは読者への問いかけで終わる）
...
#### 【note逆導線】テンプレート（本日のカンファレンスの直後、参照セクションの直前に配置）

ブログからnoteへの逆導線。ブログは「解説・情報」、noteは「個人の想い・エッセイ」という役割分担を読者に示す。CTA（退職代行）の後に置くことで、収益導線を邪魔しない。

  <!-- ========== note誘導 ========== -->
  <!-- wp:group {"className":"is-style-big_icon_memo","layout":{"type":"constrained"}} -->
  <div class="wp-block-group is-style-big_icon_memo"><!-- wp:paragraph -->
  <p>この記事のエッセイ版をnoteで公開しています。ブログでは[テーマ]を解説しましたが、noteでは看護師26年の筆者が[作品/テーマ]に重ねた<strong>個人的な想い</strong>を書いています。</p>
  <!-- /wp:paragraph -->
  <!-- wp:paragraph -->
  <p>▶ <a href="https://note.com/hikaru_kango26" target="_blank" rel="noopener">[noteエッセイのタイトル]｜ひかる</a></p>
  <!-- /wp:paragraph --></div><!-- /wp:group -->

**ルール：**
- SWELLの「メモ」装飾ボックス（is-style-big_icon_memo）で控えめに表示
- noteリンクは https://note.com/hikaru_kango26 を基本とし、公開後に記事個別URLに差し替え
- ブログは「解説」、noteは「想い」という対比を明示する

**導線の全体循環：**
note記事（中盤）→ ブログ（詳しい解説）→ note（エッセイ版）→ X投稿 → note → ...
...
### note用（4枚）
ブログ用と同じフォーマットだが、シーン描写に「Delicate soft lighting, gentle composition」を加えて柔らかい雰囲気にする。

**⚠️ 1枚目（ヘッダー＝サムネイル）だけは、プロンプトにnoteタイトルのテキストを入れる：**
末尾に `with bold Japanese text overlay "[noteタイトルの短縮版]" displayed prominently` を追加すること。

1. ヘッダー画像（サムネイル）← タイトルテキスト入り必須
2. 見出し1〜2のイメージ（体験談・感情）
3. 見出し3〜4のイメージ（内省・変化）
4. 末尾画像（余韻・希望）
...
ファイル保存先：ユーザーのObsidian Vault直下
キーワード_YYYY-MM-DD_v1.0.docx

章構成：
  【1】noteエッセイ（見出し4か所付き）
  【2】なのバナナ画像プロンプト（ブログ用7枚）
  【2-2】なのバナナ画像プロンプト（note用4枚）
  【3】画像ファイル名＆alt属性
...
Node.jsの docx ライブラリを使ってスクリプトを書き、実行する。
...
- [ ] 処方箋が「気づく・動く・守る」の3軸になっている
- [ ] ブログHTMLの末尾にnoteエッセイへの導線が入っている
- [ ] MetaTitle・MetaDescriptionが出力済みか
- [ ] NotebookLMプロンプトが出力済みか
- [ ] なのバナナ画像プロンプトが出力済みか
- [ ] 画像ファイル名が出力済みか

---

> このチェックリストをすべて通過したらWordファイルに出力して完了とする。
